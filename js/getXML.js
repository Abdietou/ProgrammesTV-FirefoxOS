...
function pr_json(pr) {
    ...
    try {
        programmeSousTitre = "(" + pr.getElementsByTagName("sub-title")[0].textContent + ")";
    } catch (exceptiondesc) {
        programmeSousTitre = "";
    }
    try {
        programmeDesc = pr.getElementsByTagName("desc")[0].textContent;
    } catch (exceptiondesc) {
    ...
    resPr['programmeLength'] = programmeLength[0].textContent
...
function xml2json(xml) {
    ...
    for (j = 0; j < programmes.length; j++) {
        if (parseInt(programmes[j].getAttribute("channel")) < 25 && programmes[j].getAttribute("start") < d)
            resXml["programmes"].push(pr_json(programmes[j]))
    }
    channels = xml.getElementsByTagName("channel")
    for (j = 0; j < channels.length; j++) {
        if (parseInt(channels[j].getAttribute("id")) < 25)
...
function process_xml(xmlData) {
    ...
    var channels = xmlData['channels'] //getElementsByTagName("channel");//balise chaine - liste de toutes les chaines
    ...
    for (i = 0; i < channels.length; i++) {
        ...
        var channelId = channels[i]["channelId"] //.getAttribute("id");
        var channelName = channels[i]["channelName"] //.getElementsByTagName("display-name")[0].textContent;
        ...
        var programmes = xmlData["programmes"] //.getElementsByTagName("programme");
        outer: for (j = 0; j < programmes.length; j++) {

            //On stocke le numéro de la chaîne courante
            var programmeId = programmes[j]["programmeId"] //.getAttribute("channel");

            // On sauvegarde la date et l'heure de fin du programme courant
            var programmeStop = programmes[j]["programmeStop"] //.getAttribute("stop");
            ...
            if ((programmeId == channelId) && (new Number(dateJour) < new Number(datestop)) && token == 0) {
                var programmeStart = programmes[j]["programmeStart"] //.getAttribute("start");
                ...
                var programmeTitre = programmes[j]["programmeTitre"] //.getElementsByTagName("title")[0].textContent;

                //On teste la présence du sous titre                        
                var programmeSousTitre = null;
                try {
                    programmeSousTitre =  programmes[j]["programmeSousTitre"] //.getElementsByTagName("sub-title")[0].textContent+")";
                } catch (exceptiondesc) {
                ...
                try {
                    programmeDesc = programmes[j]["programmeDesc"]; //.getElementsByTagName("desc")[0].textContent;
                } catch (exceptiondesc) {
                ...
                programmeType = programmes[j]["programmeType"] //.getElementsByTagName("category")[0].textContent;
                programmeCredits = programmes[j]["programmeCredits"] //.getElementsByTagName("credits")[0];
                programmeDirecteur = programmes[j]["programmeDirecteur"];
                programmeActeurs = programmes[j]["programmeActeurs"];
                programmeDate = programmes[j]["programmeDate"] //.getElementsByTagName("date")[0].textContent;
                var programmeLength = programmes[j]["programmeLength"] //.getElementsByTagName("length");
                var lenghtUnit = programmes[j]["lenghtUnit"] //.getAttribute("units");
                ...
                for (q = 0, p = j + 1; q < 3 && programmeId == programmes[p]["programmeId"]; q++, p++) {
                    ...
                    baliseCourante.textContent = programmes[p]["programmeTitre"];
                    ...
                    try {
                        baliseCourante.textContent = programmes[p]["programmeSousTitre"];
                    } catch (exceptiondesc) {
                        //Si la balise sous titre est absente, on met null pour ne pas bloquer l'exécution du code
                    ...
                    programmeStartNext = programmes[p]["programmeStart"];
                    ...
                    programmeLengthNext = programmes[p]["programmeLength"];
                    lenghtUnitNext = programmes[p]["lenghtUnit"];

                    //On écrit dans cette balise les informations sur l'heure de début du programme
                    baliseCourante.textContent = debutNext + " (" + programmeLengthNext + " " + lenghtUnitNext + ")";
            ...
            var programmeDeb = programmes[j]["programmeStart"];
            ...
            if ((programmeId == channelId) && (new Number(dateSoir) <= new Number(datestart)) && tokenS == 0) {
                var programmeStartS = programmes[j]["programmeStart"];
                ...
                var programmeTitreS = programmes[j]["programmeTitre"];

                //On teste la présence du sous titre                        
                var programmeSousTitreS = null;
                try {
                    programmeSousTitreS = "(" + programmes[j]["programmeSousTitre"] + ")";
                } catch (exceptiondesc) {
                    //Si la balise sous titre est absente, on met null pour ne pas bloquer l'exécution du code
                ...
                //On teste la présence d'une description
                try {
                    programmeDescS = programmes[j]["programmeDesc"];
                } catch (exceptiondesc) {
                    //Si la balise description est absente, on informe l'utilisateur
                ...
                programmeTypeS = programmes[j]["programmeType"] //.getElementsByTagName("category")[0].textContent;
                programmeCreditsS = programmes[j]["programmeCredits"] //.getElementsByTagName("credits")[0];
                programmeDirecteurS = programmes[j]["programmeDirecteur"];
                programmeActeursS = programmes[j]["programmeActeurs"];


                try {
                    programmeDateS = programmes[j]["programmeDate"];
                ...
                var programmeLengthS = programmes[j]["programmeLength"] //.getElementsByTagName("length");
                var lenghtUnitS = programmes[j]["lenghtUnit"] //.getAttribute("units");
                ...
                for (q = 0, p = j + 1; q < 3 && programmeId == programmes[p]["programmeId"] && p < programmes.length - 1; q++, p++) {
                    ...
                    //On attache le titre de l'émission suivante
                    baliseCouranteS.textContent = programmes[p]["programmeTitre"];
                    ...
                    //On essaye de charger le sous titre.
                    try {
                        baliseCouranteS.textContent = programmes[p]["programmeSousTitre"];
                    } catch (exceptiondesc) {
                        //Si la balise sous titre est absente, on met null pour ne pas bloquer l'exécution du code
                    ...
                    //Formattage de la variable start pour le début de programme
                    programmeStartNextS = programmes[p]["programmeStart"];
                    ...
                    programmeLengthNextS = programmes[p]["programmeLength"];
                    lenghtUnitNextS = programmes[p]["lenghtUnit"];


                    //On écrit dans cette balise les informations sur l'heure de début du programme
                    baliseCouranteS.textContent = debutNextS + " (" + programmeLengthNextS + " " + lenghtUnitNext + ")";
        ...
        tmp.textContent = debut + " (" + programmeLength + " " + lenghtUnit + ")";
        ...
        //Insertion de la date, l'heure de début et la durée du programme
        tmpS.textContent = debutS + " (" + programmeLengthS + " " + lenghtUnitS + ")";
...
function init() {
    document.getElementById('listIcon').addEventListener("click", function() {
        document.getElementById('listeProgramme').style.display = "block";
    ...
    for (var i = 0; i < document.getElementsByClassName("info_complementaire").length; i++) {
        ...
        document.getElementsByClassName("info_complementaire")[i].addEventListener("click", function() {
            this.style.height = "auto";
        }, false);
        document.getElementsByClassName("info_complementaire")[i].addEventListener("dblclick", function() {
